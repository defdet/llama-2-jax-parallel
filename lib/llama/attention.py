...
from .ring_attention import ring_attention_standard as ring_attention
...
def forward_attention(params: Attention, src_seq: Array, dst_seq: Array, qk_mask: Array, *, rotary_values: RotaryValues, kv_cache: KVCache | None=None, model_config: ModelConfig) -> tuple[Array, KVCache | None]:
    ...
    specs_tuple_ring = (P(*name_tuple_k), P(*name_tuple_k), P(*name_tuple_k), P(*name_tuple_k), P(*name_tuple_id))
    ...
    if attn_impl == 'flash':
        ...
        qkv = shard_map(partial(ring_attention, axis_name='b', float32_logits=True, sm_scale=math.sqrt(model_config.d_k), debug=False, causal=True), mesh=mesh_k, in_specs=specs_tuple_ring, out_specs=P(*name_tuple_k), check_rep=False)(q, k, v, attention_bias, segment_ids)
